...
- 已安装 `rtunnel` 并在 `PATH` 中（`which rtunnel` 能找到）；如使用 WebSocket 隧道，也可安装 `wstunnel`
...
1. 「新建隧道」里填：
   ...
   - **额外参数**（可选）：如 `--secure`
   - **隧道命令**（可选）：自定义 `rtunnel` 可执行文件路径或命令名，留空则自动从 PATH 搜索；填 `wstunnel` 时会按兼容模式启动
   - **使用 sudo 启动**（可选）：启动 / 重启时弹出 sudo 密码框，密码只用于本次请求，不保存
   - **跳过启动前直连校验**（可选）：DNS 或网络环境会误伤直连探测时使用
2. 点「添加」→ 点「启动」。
3. 启动成功后（状态变「运行中」），条目里会出现一条**高亮的 ssh 指令**
   `ssh -p 4444 root@127.0.0.1`，点一下即可复制粘贴到终端登录。

默认等价命令：`rtunnel <远程URL> <本地端口> [额外参数]`
...
如果自定义命令填写 `wstunnel`，则等价于：

```bash
wstunnel -t 127.0.0.1:<本地端口>:127.0.0.1:22 [额外参数] <远程URL>
```

这里显式绑定 `127.0.0.1`，避免 `wstunnel` 只监听 `::1` 时，前端展示的
`ssh -p <本地端口> root@127.0.0.1` 无法连接。

勾选「使用 sudo 启动」后，管理器会在启动 / 重启时弹出密码框，并通过
`sudo -S -- <隧道命令> ...` 启动。密码不会写入 `tunnels.json`、日志或运行状态文件。

## 修改已添加的隧道

每条隧道都有「**编辑**」按钮：就地展开表单，可改名称 / URL / 用户名 / 端口 /
额外参数 / 隧道命令 / sudo 启动 / 是否跳过直连校验。

- **未运行**：保存后下次「启动」用新配置。
- **运行中**：会多出「**保存并重启**」按钮——先保存再用新配置重启（按配置执行直连校验）。
  只点「保存」则已在跑的旧进程不受影响。
- **sudo 隧道**：点「启动」或「保存并重启」时才询问密码；编辑页不会保存密码。
...
如果远程 URL 的 DNS 只能在特殊网络环境下解析，或网关本身不适合被 HTTP 探测，
可以在该隧道编辑页勾选「跳过启动前直连校验」。跳过后管理器会直接启动 `rtunnel`，
但后续仍会通过本地 SSH banner 探测展示「运行中 / 已断开」状态。
...
| 文件 | 位置 | 是否跨设备同步 | 内容 |
|---|---|---|---|
| `tunnels.json` | 脚本同目录 | ✅ 是（如果脚本在 iCloud/Dropbox 同步目录） | 隧道列表（URL/端口/名字/用户/参数/自定义命令/sudo/直连校验设置） |
